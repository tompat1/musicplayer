...
const RADIO_BROWSER_API_BASE_URL = 'https://all.api.radio-browser.info/json';
...
const fetchRadioBrowserJson = async (path, signal) => {
  const response = await fetch(`${RADIO_BROWSER_API_BASE_URL}${path}`, { signal });
  if (!response.ok) throw new Error(`Radio Browser proxy returned ${response.status}`);
...
const reportRadioStationClick = async (station) => {
  ...
  try {
    await fetch(`${RADIO_BROWSER_API_BASE_URL}/url/${encodeURIComponent(stationId)}`);
  } catch {
...
function RadioBrowserPanel({
  ...
  const searchStations = useCallback(async (signal) => {
    ...
    try {
      ...
      const data = await fetchRadioBrowserJson(`/stations/search?${params.toString()}`, signal);
